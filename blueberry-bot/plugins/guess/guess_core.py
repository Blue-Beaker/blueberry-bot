...
class GuessSession:
    ...
    def __init__(self,mapData:MapData,map_exported_data:dict,guesses_per_info:int=3,starting_info:int=1) -> None:
    ...
    def guesses_for_next_info(self):
        return (self.revealed_info.__len__()-self.starting_info+1)*self.guesses_per_info
...
class GuessManager:
    ...
    def _start_guess(self) -> GuessSession:
        ...
        self.session=GuessSession(map,getMapJsonFromPath(map.filePath),self.guesses_per_info,self.starting_info)
        return self.session
    ...
    def dump(self):
        ...
        if(self.session):
            ...
            dumpdata["starting_info"]=self.session.starting_info
        else:
    ...
    def load(cls,dumpdata:dict[str,str|list[str]|int]):
        ...
        if mapData:
            ...
            info=dumpdata.get("revealed_info",{})
            ...
            guesses=dumpdata.get("guesses",0)
            if(isinstance(guesses,int)):
                session.guesses=guesses
                
            guesses_per_info=dumpdata.get("guesses_per_info",3)
            if(isinstance(guesses_per_info,int)):
                session.guesses_per_info=guesses_per_info
                
            starting_info=dumpdata.get("starting_info",1)
            if(isinstance(starting_info,int)):
